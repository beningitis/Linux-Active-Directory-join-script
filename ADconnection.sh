    ...
    if [ "$sshsec" = "yes" ]; then
        if [ -f /etc/ssh/login.group.allowed ] </dev/null >/dev/null 2>&1; then
            ...
            if [ "$localadmin" = "null" ]; then
                ...
                cat /etc/passwd | grep home | while read locaussh; do
                    echo $locaussh | grep home | grep bash | cut -d ':' -f1 | sudo tee -a sudo tee -a /etc/ssh/login.group.allowed
                done
...
entry_cache_nowait_percentage = 75 " | sudo tee -a /etc/sssd/sssd.alternatives
        ...
        if [ "$usesasl" = "no" ]; then
            ...
            if [ "$usesasl" = "yes" ]; then
                ...
                if [ "$sasl" = "null" ]; then
                    ...
                    if ! ls "$cacer"; then
                        echo "No root CA found, check your path to file"
                    else
    ...
    if [ $states = 12 ]; then
        ...
        homedir=$(grep homedir /etc/pam.d/common-session | grep 0077 | cut -d '=' -f3 | head -1)
        if [ "$homedir" = "0077" ] </dev/null >/dev/null 2>&1; then
...
entry_cache_nowait_percentage = 75 " | sudo tee -a /etc/sssd/sssd.alternatives
    ...
    if [ "$usesasl" = "no" ]; then
        ...
        if [ "$usesasl" = "yes" ]; then
            ...
            case $yn in
            [Yy]*)
                if [ -f "/usr/share/ca-certificates/root/*.cer" ]; then
                    cacert=$(ls /usr/share/ca-certificates/root/ | grep .cer | head -1)
                    echo "Type in address of your Domaincontroller: ex: dc01.com"
                    read -r yourDC
                    clear
...
linuxclient() {
    ...
    clear
    #### OS detection ####
    if [ "$TheOS" = "Zorin" ] </dev/null >/dev/null 2>&1; then
        Zorin_os
    else
        if [ "$TheOS" = "Fedora" ] </dev/null >/dev/null 2>&1; then
            ...
            if [ "$TheOS" = "CentOS" ] </dev/null >/dev/null 2>&1; then
                ...
                if [ "$TheOS" = "Debian" ] </dev/null >/dev/null 2>&1; then
                    ...
                    if [ "$TheOS" = "SUSE" ] </dev/null >/dev/null 2>&1; then
                        echo "SUSE detected"
                        SUSEclient
                    else
...
UbuntU() {
    ...
    if ! sudo apt-get -qq install realmd adcli sssd ntp curl -y && sudo apt-get -qq install -f -y; then
    ...
    if [ "$var" -eq "14" ]; then
        ...
        if [ "$encrypt" = "null" ] || [ "$encrypt" = "no" ]; then
            if ! sudo realm join --verbose --user="$ADMIN" "$DOMAIN" "$OUSPECIFIED" --install=/; then
            ...
            if [ "$encrypt" = "yes" ]; then
                if [ -f private_key.pem ] && [ -f public_key.pem ]; then
                    ...
                    if ! echo $enc | sudo realm join -v -U "$ADMIN" "$DOMAIN" "$OUSPECIFIED" --install=/; then
                        echo "${RED_TEXT}AD join failed.please check your errors with journalctl -xe${END}"
                        enc=$(null)
                        exit
                    fi
        ...
        if [ "$var" -eq "16" ]; then
            ...
            if [ "$var" -eq "17" ] || [ "$var" -eq "18" ] || [ "$var" -eq "19" ] || [ "$var" -eq "20" ] || [ "$var" -eq "21" ] || [ "$var" -eq "22" ]; then
                echo "${INTRO_TEXT}Detected Ubuntu $var${END}"
                sleep 1
                clear
                if [ "$var" -eq "19" ] || [ "$var" -eq "20" ] || [ "$var" -eq "21" ] || [ "$var" -eq "22" ]; then
                    if [ -f /etc/apt/sources.list.d/aroth-ubuntu-ppa-eoan.list ]; then
                        sudo apt-get update
    ...
    if [ "$var" -eq "14" ]; then
        ...
        if [ "$encrypt" = "null" ] || [ "$encrypt" = "no" ]; then
            ...
            if [ "$encrypt" = "yes" ]; then
                if [ -f private_key.pem ] && [ -f public_key.pem ]; then
                    enc=$(sudo openssl pkeyutl -decrypt -inkey private_key.pem -in encrypted.dat)
                    if ! echo $enc | sudo realm join -v -U "$ADMIN" "$DOMAIN" --install=/; then
                        echo "${RED_TEXT}AD join failed.please check your errors with journalctl -xe${END}"
                        enc=$(null)
                        exit
                    fi
        ...
        if [ "$var" -eq "16" ]; then
            ...
            if [ "$var" -eq "14" ] || [ "$var" -eq "15" ] || [ "$var" -eq "16" ] || [ "$var" -eq "17" ]; then
                echo "${INTRO_TEXT}Detected Zorin ${END}"
                sleep 1
                clear
                if [ "$var" -eq "15" ] || [ "$var" -eq "16" ]; then
                    if [ -f /etc/apt/sources.list.d/aroth-ubuntu-ppa-eoan.list ]; then
                        sudo apt-get update
                        #sudo apt-get --only-upgrade install adcli
                        sudo apt install adcli -y
...
ubuntuserver14() {
    ...
    encrypt=$(sudo grep ENCRYPTEDPASSWD readfile | awk '{print $3}')
    if [ "$encrypt" = "null" ] || [ "$encrypt" = "no" ]; then
        if ! sudo realm join --verbose --user="$DomainADMIN" "$DOMAIN" --install=/; then
            echo "${RED_TEXT}AD join failed.please check your errors with journalctl -xe${END}"
            exit
        fi
    else
        if [ "$encrypt" = "yes" ]; then
            if [ -f private_key.pem ] && [ -f public_key.pem ]; then
                enc=$(sudo openssl pkeyutl -decrypt -inkey private_key.pem -in encrypted.dat)
                if ! echo $enc | sudo realm join -v -U "$DomainADMIN" "$DOMAIN" --install=/; then
                    echo "${RED_TEXT}AD join failed.please check your errors with journalctl -xe${END}"
...
failcheck() {
    ...
    if ! realm discover </dev/null >/dev/null 2>&1; then
        ...
        homedir=$(grep homedir /etc/pam.d/common-session | grep 0077 | cut -d '=' -f3 | head -1)
        if [ "$homedir" -eq "0077" ] </dev/null >/dev/null 2>&1; then
            echo Checking PAM configuration.. "${INTRO_TEXT}OK${END}"
        else
            echo Checking PAM configuration.. "${RED_TEXT}FAIL${END}"
        fi
...
failcheck_yum() {
    ...
    echo "-------------------------------------------------------------------------------------"
    echo ""
    if ! realm discover "$therealm"; then
        echo "realm not found"
    else
        echo ""
        therealm=$(realm discover | grep -i realm-name | awk '{print $2}')
        ...
        if [ -f /etc/ssh/login.group.allowed ]; then
...
ldaplook() {
    ...
    if [ "$ldaptools" = dap-uti ]; then
        ...
        clear
        if ! sudo apt-get install ldap-utils curl -y; then
            echo "install failed"
            exit
        else
...
ldaplookyum() {
    ...
    if [ "$ldaptools" = dap-uti ]; then
        ...
        clear
        if ! sudo yum install ldap-utils -y; then
            echo "install failed"
            exit
        else
            echo "${NUMBER}please edit in ldap.conf the lines BASE and URI ${END}"
...
Reauthenticate() {
    ...
    if [ -f /etc/sssd/sssd.conf ]; then
        ...
        [Yy]*)
            ...
            if ! sudo realm leave "$SSSD" --remove; then
